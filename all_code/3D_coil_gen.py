...
def coil_coord_gen(regcoil_p,nescin_p,coils_per_p,p,thickness):
  ...
  #SUCCESSFULLY COVERTED ALL ABOVE
...
#    for c in range (np.size(potential[0,:])): 
    ...
    #Write zeta,theta and potential values to a file: 
  ...
  #  plt.colorbar(c1)
    ...
    #WORKS UP TO HERE AS OF 14/10/24
...
#        plt.plot(contours_zeta[j],contours_theta[j],'r',linewidth=2)
 ...
 #       ax = plt.gca(projection='3d')
  #      ax.plot_surface(coils_x[j]+offset,coils_y[j],coils_z[j],edgecolor='none',facecolor=colors[nextColor,:],ambient=ambientStrength,diffuse=diffuseStrength)
            for k in range(4):
 #           ax.plot3D(coils_x[j][:,k]+offset,coils_y[j][:,k],coils_z[j][:,k],edgeColor,linewidth=0.1)
                centre = "coords/c_" + str(j) + ".csv"
                xfile = "coords/x_" + str(j+1) + "_" + str(k+1) + ".csv"
                yfile = "coords/y_" + str(j+1) + "_" + str(k+1) + ".csv"
                zfile = "coords/z_" + str(j+1) + "_" + str(k+1) + ".csv"
